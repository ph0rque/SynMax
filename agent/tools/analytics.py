...
def cluster_pipelines_monthly(executor: DuckDBExecutor, parquet_path: str, k: int = 5, top_k_pipelines: int = 50) -> pa.Table:
    pipelines = _top_pipelines(executor, parquet_path, top_k_pipelines)
    if not pipelines:
    ...
    out = pd.DataFrame({"pipeline_name": pivot.index.tolist(), "cluster": labels.tolist()})
